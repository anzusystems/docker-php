...
ENV MAKEFLAGS="-j4"
# Common envs
ENV DOCKER_APP_EXCLUDED_ENV_VARS="HOME|LOGNAME|OLDPWD|PWD|SHELL|SHLVL|TERM" \
...
ENV PHP_DATE_TIMEZONE="UTC" \
    ...
    PHP_REGISTER_ARGC_ARGV=0 \
    PHP_SESSION_COOKIE_PARTITIONED=0 \
...
RUN apt-get update && \
    ...
    docker-php-ext-install -j$(nproc) \
        ...
        pdo_mysql \
        zip && \
    # Install opcache only for PHP < 8.5 (bundled in 8.5+)
...
RUN apt-get update && \
    apt-get install -y \
        ${PECL_BUILD_DEPS} && \
    yes '' | MAKEFLAGS="-j$(($(nproc)+2))" pecl install apcu-${PECL_APCU_VERSION} && \
